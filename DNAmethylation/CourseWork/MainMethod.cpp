...
vector<Human> MainMethod::readCSV( string pathForMatrixHormone, string pathForCase, int numX, int numY )
{
...
int MainMethod::getNumberOfControl()
{
	int res = 0;
...
void MainMethod::indicateProgress( int xi, int maxXi )
{
	if (xi % 10 == 0) cout << xi / 10 << endl;
}

MainMethod::MainMethod( string pathToData, string pathToDataBool, int numHumans /*= 684*/, int numMRA /*= 1000*/, int maxLenghtOfGraph /*= 500*/ )
{
	numX = numHumans;
	numY = numMRA;
...
MainMethod::~MainMethod( void )
{

}

vector<SortedListOfEdge> MainMethod::execute()
{
	...
	for (int xi = 0; xi < numberOfMRA; xi++) 
	{
		...
		for (int yi = xi+1; yi < numberOfMRA; yi++) 
		{
			...
			vector<double> x(hulfOfHealphHumans);
			vector<double> y(hulfOfHealphHumans);
...
vector<bool> MainMethod::getSickVector()
{
